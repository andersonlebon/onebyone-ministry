  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.5)" }} onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
        className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e3d9ce]">
          <h3 className="text-base text-[#474747]">{project ? "Edit Project" : "New Project"}</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.status} onChange={(e) => set("status", e.target.value as Project["status"])} className="w-full px-3 py-2.5 rounded-xl border text-sm bg-white focus:outline-none focus:border-[#6E9277]" style={{ borderColor: "rgba(110,146,119,0.3)" }}>
                {STATUSES.map((s) => <option key={s}>{s}</option>)}
              </select>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-[#474747] mb-1.5">Short Description</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#474747] mb-1.5">Full Description</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-[#474747] mb-1.5">Location</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-[#474747] mb-1.5">Year(s)</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-[#474747] mb-1.5">Impact</label>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl text-[#474747]">Projects</h1>
          <p className="text-sm text-[#7a7068]">{projects.length} total · {projects.filter(p => p.status === "Active").length} active</p>
        </div>
        ...
        <AnimatePresence>
          {projects.map((project, i) => (
            <motion.div key={project.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ delay: i * 0.04 }}
              className="bg-white rounded-2xl border border-[#e3d9ce] flex items-center gap-4 px-5 py-4 hover:shadow-sm">
              <img src={project.img} alt="" className="w-14 h-14 rounded-xl object-cover flex-shrink-0 bg-[#e3d9ce]" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#474747] truncate">{project.title}</p>
                <p className="text-xs text-[#7a7068] truncate mt-0.5">{project.desc}</p>
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-xs text-[#7a7068]">{project.category} · {project.location}</span>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <motion.button whileHover={{ scale: 1.1 }} onClick={() => { setEditing(project); setShowModal(true); }} className="p-2 rounded-lg hover:bg-[#EFE7DB] text-[#7a7068]"><Pencil size={14} /></motion.button>
