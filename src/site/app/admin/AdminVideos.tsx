  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.5)" }} onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
        className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e3d9ce]">
          <h3 className="text-base text-[#474747]">{video ? "Edit Video" : "Add Video"}</h3>
          <button onClick={onClose}><X size={16} className="text-[#7a7068]" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-[#474747] mb-1.5">YouTube Video ID</label>
            ...
            <p className="text-xs text-[#7a7068] mt-1">Paste the ID from the YouTube URL (after ?v=)</p>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-[#474747] mb-1.5">Duration</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#474747] mb-1.5">Thumbnail URL</label>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl text-[#474747]">Videos</h1>
          <p className="text-sm text-[#7a7068]">{videos.length} videos in library</p>
        </div>
        ...
        <AnimatePresence>
          {videos.map((video, i) => (
            <motion.div key={video.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-white rounded-2xl border border-[#e3d9ce] overflow-hidden hover:shadow-sm">
              <div className="relative aspect-video bg-[#e3d9ce]">
              ...
              <div className="p-4">
                <p className="text-xs font-semibold text-[#474747] mb-1">{video.title}</p>
                <p className="text-xs text-[#7a7068] mb-3">{video.category}</p>
                <div className="flex gap-2">
                  <motion.button whileHover={{ scale: 1.08 }} onClick={() => { setEditing(video); setShowModal(true); }}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-xs border border-[#e3d9ce] hover:bg-[#EFE7DB] text-[#474747]">
                    <Pencil size={11} /> Edit
                  </motion.button>
