    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e3d9ce]">
          ...
          <button onClick={onClose} className="text-[#7a7068]">
            <X size={16} />
          </button>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border text-sm bg-white focus:outline-none focus:border-[#6E9277]"
              style={{ borderColor: "rgba(110,146,119,0.3)" }}
            >
        ...
        <div className="px-6 py-4 border-t border-[#e3d9ce] flex gap-3 justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl text-sm text-[#474747] border border-[#e3d9ce] hover:bg-[#EFE7DB]"
          >
            Cancel
          </button>
          <motion.button
...
export default function AdminPhotos() {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {CATEGORIES.map((cat) => (
          <button
            ...
            style={{
              backgroundColor: activeCategory === cat ? "#6E9277" : "#ffffff",
              color: activeCategory === cat ? "#ffffff" : "#474747",
              border: activeCategory === cat ? "none" : "1px solid #e3d9ce",
            }}
          >
            {cat}
          </button>
      ...
      {loading ? (
        <div className="py-16 text-center text-sm text-[#7a7068]">Loading photos from Supabase...</div>
      ) : (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                  <motion.button
                    ...
                    className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#474747]"
                  >
                    <Pencil size={13} />
                  </motion.button>
                  <motion.button
                    ...
                    className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-red-500"
                  >
                    <Trash2 size={13} />
                  </motion.button>
                </div>
